...
namespace ICS.WebApplication.Commands.Read
{
	...
	public sealed class InvitationWriteCommand
    {
        ...
        public async Task<Guid> AddOrUpdateStateRegistrationAsync(Guid invitationId, StateRegistrationDto stateRegistrationDto)
        {
            ...
            var alienStateRegistrtionId = await _alienWriteCommand.AddOrUpdateStateRegistrationAsync(invitedAlienId, stateRegistrationDto);

            await _domainContext.SaveChangesAsync();

            return alienStateRegistrtionId;
        }
